...
3. **Build the Sound** — Identify consonants in Level 1, then match words to their first vowel sound in Level 2.
...
The two map activities come first as foundational practice. More advanced activities remain available without requiring completion of the maps; Build the Sound retains its own Level 2 unlock requirement.
...
### Activities 4 and 5: Question Layout

**Mystery Sound (Activity 4)** and **Vowel Detective (Activity 5)** use larger question areas to make better use of the available screen height:

- Question instructions, phonetic features, and mystery clues use larger text.
- Answer buttons expand into the available space and display larger IPA symbols.
- Mystery Sound's answer choices are no longer restricted to a small internally scrolling area.
- On narrow screens, choices use two columns and the activity height follows its content.

These layouts use the shared `.expanded-quiz` styles in `my-app/src/App.css`. The 30-second timers, scoring, feedback, and navigation behavior are unchanged.
...
- A score of **70% or above (at least 7 out of 10)** unlocks Level 2. The Level 1 summary includes **Continue to Level 2** after unlocking.
...
- `build_the_sound/Activity3_BuildTheSound.jsx` coordinates level selection, the 70% unlock, and saved results.
...
Exactly 70% unlocks the next level in Activities 2 and 3. The minimum passing scores are 18/25 and 28/40 in Consonant Map Drop, and 7/10 in Build the Sound.
